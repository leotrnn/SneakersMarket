...
CREATE TABLE `FAVORIS` (
  ...
  CONSTRAINT `FAVORIS_FK` FOREIGN KEY (`idUser`) REFERENCES `USERS` (`idUser`),
  CONSTRAINT `FAVORIS_FK_1` FOREIGN KEY (`idSneaker`) REFERENCES `SNEAKERS` (`idSneaker`)
...
CREATE TABLE `TRANSACTIONS` (
  ...
  CONSTRAINT `TRANSACTIONS_FK_2` FOREIGN KEY (`idSneaker`) REFERENCES `SNEAKERS` (`idSneaker`),
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`dbeaver_admin`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `VIEW_TRANSACTIONS` AS select `T`.`idTransaction` AS `idTransaction`,`S`.`idSneaker` AS `idSneaker`,`S`.`sizeSneaker` AS `sizeSneaker`,`S`.`prixSneaker` AS `prixSneaker`,`S`.`imgSneaker` AS `imgSneaker`,`S`.`descriptionSneaker` AS `descriptionSneaker`,`MS`.`nomSStatut` AS `nameSStatut`,`M`.`nameBrand` AS `nameBrand`,`MO`.`nameModel` AS `nameModel`,`V`.`nomVisibilite` AS `nameVisibility`,`UU`.`idUser` AS `idBuyer`,`UU`.`nameUser` AS `nameBuyer`,`UU`.`surnameUser` AS `surnameBuyer`,`UU`.`emailUser` AS `emailBuyer`,`UV`.`idUser` AS `idSeller`,`UV`.`nameUser` AS `nameSeller`,`UV`.`surnameUser` AS `surnameSeller`,`UV`.`emailUser` AS `emailSeller`,`PT`.`nomMeetPoint` AS `pointRencontre`,`TS`.`nomStatut` AS `nameStatut` from (((((((((`TRANSACTIONS` `T` join `SNEAKERS` `S` on(`T`.`idSneaker` = `S`.`idSneaker`)) join `SSTATUT` `MS` on(`S`.`idSStatut` = `MS`.`idSStatut`)) join `MARQUES` `M` on(`S`.`idMarque` = `M`.`idMarque`)) join `MODELE` `MO` on(`S`.`idModele` = `MO`.`idModele`)) join `VISIBILITE` `V` on(`S`.`idVisibilite` = `V`.`idVisibilite`)) join `USERS` `UU` on(`T`.`idBuyer` = `UU`.`idUser`)) left join `USERS` `UV` on(`T`.`idSeller` = `UV`.`idUser`)) join `PTRENCONTRE` `PT` on(`T`.`idMeetPoint` = `PT`.`idMeetPoint`)) join `TSTATUT` `TS` on(`T`.`idTStatut` = `TS`.`idTStatut`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`dbeaver_admin`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `VIEW_USERS` AS select `USERS`.`idUser` AS `idUser`,`USERS`.`nameUser` AS `nameUser`,`USERS`.`surnameUser` AS `surnameUser`,`USERS`.`emailUser` AS `emailUser`,`USERS`.`pwdUser` AS `pwdUser`,`SCHOOLS`.`nameSchool` AS `nameSchool`,`USTATUT`.`nomUStatut` AS `nomUStatut` from ((`USERS` join `SCHOOLS`) join `USTATUT`) where `USERS`.`idSchool` = `SCHOOLS`.`idSchool` and `USERS`.`idUStatut` = `USTATUT`.`idUStatut` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2024-04-25 16:41:50
